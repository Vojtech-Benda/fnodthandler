...
logger.info(f"movescu destination is {receiver_aetitle}:{receiver_store_port}")
...
def download_dcm(pacs: dict, series_uids: list):
    ...
    for serie_uid in series_uids:
        ...
        logger.debug(f"creating download directory \"{output_dir}\"")
        ...
        try: 
            ...
            logger.error(f"script movescu.py failed with exit code {er.returncode}")
            logger.error(f"script arguments {pacs}, download dir {output_dir}, series uid {serie_uid}")
    ...
    logger.info(f"downloaded {len(downloaded_uids)} out of {len(series_uids)} series")
    
    result = TaskResult()
    
    if len(failed_uids) != 0:
        result.mark_warning("C-MOVE: some files failed to download", stdout=sub_result.stdout)
        logger.warning(result.format_result())
        logger.warning(f"failed uids:\n{"\n".join(failed_uids)}")
        return result
        ...
        logger.error(result.format_result())
        logger.error(f"failed uids:\n{"\n".join(failed_uids)}")
    else:
